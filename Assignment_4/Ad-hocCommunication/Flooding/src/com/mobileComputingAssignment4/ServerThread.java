                ...
                if (message.equals("DISCOVER_NODE_REQUEST")) {
                   // byte[] sendData = "DISCOVER_NODE_RESPONSE".getBytes();
                    ...
                    if (!clientModeOn && seqNr != seqNrTemp)
                    {
                      ...
                      //  Message fwdMsg = new Message(msg.getSequenceNumber(), msg.getHopCount() + 1, msg.getMessageData());
                        //System.out.println(getClass().getName() + " New message data: "+ new String(ackMsg.getMessageData()));
                        //DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, packet.getAddress(), packet.getPort());

                      //  DatagramPacket sendPacket = new DatagramPacket(fwdMsg.toByte(), fwdMsg.getLength(), InetAddress.getByName("192.168.132.255"), packet.getPort());
                        //Thread.sleep(1000);
                     //   sendSock.send(sendPacket);
                     //   System.out.println(getClass().getName() + "Message forwarded");
                       // System.out.println(getClass().getName() + " SequenceNumber: " + fwdMsg.getSequenceNumber() + ", HopCount: " + fwdMsg.getHopCount() + (", messageData: " + new String(fwdMsg.getMessageData())));
